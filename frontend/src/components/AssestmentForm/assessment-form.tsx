...
const AssessmentForm: React.FC<AssessmentFormProps> = ({
  ...
  const [formData, setFormData] = useState<AssessmentAnswers>(() => {
    ...
    if (restoredFormData) {
      console.log("📥 Using restored form data from props");
    ...
    try {
      ...
      if (saved) {
        ...
        console.log("📥 Loaded from StorageEncryptor:", parsed);
    ...
    console.log("ℹ️ No saved data found, using base structure");
    return baseStructure;
  });

  const [currentSection, setCurrentSection] = useState<number>(() => {
    try {
      const saved = StorageEncryptor.getItem("currentAssessmentSection");
  ...
  // ✅ Persist answers
  useEffect(() => {
    console.log("💾 Saving progress to storage...");
  ...
  useEffect(() => {
    if (
      ...
      const hasSeenBefore = StorageEncryptor.getItem(
        "hasSeenAssessmentInstructions",
      );
      if (!hasSeenBefore) {
        setShowInstructions(true);
        hasShownInstructionsRef.current = true;
        StorageEncryptor.setItem("hasSeenAssessmentInstructions", "true");
      }
    }
  ...
  const handleStartNew = () => {
    ...
    console.log("✅ Assessment form reset complete");
  };
